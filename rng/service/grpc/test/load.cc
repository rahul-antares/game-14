...
class GrpcClient {
    public:
        ...
        uint32_t GetScaled(const uint32_t max, std::chrono::time_point<std::chrono::system_clock> startTS);
...
uint32_t GrpcClient::GetScaled(const uint32_t max, std::chrono::time_point<std::chrono::system_clock> startTS) {
    ...
    auto duration  = std::chrono::duration_cast<std::chrono::milliseconds>(now - startTS).count();
    printf ("%f calls per second\n", (double)maxIterations*1000/duration);
